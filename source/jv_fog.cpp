...
jv::Fog::Fog():
    ...
    {
        _internal_window.set_top(_y - _height - jv::Common::Camera().y());
        _internal_window.set_bottom(_y + _height - jv::Common::Camera().y());
...
void jv::Fog::update(){
    bn::point player_position = bn::point(jv::Common::Player().position().x().integer(), jv::Common::Player().position().y().integer());
    bool flag = false;
    ...
    int y_int = _y - jv::Common::cam_pos().y();
    ...
    for(int index = 0; index < _height; ++index){
        y_int = _y - jv::Common::cam_pos().y();

        int qc_index = curve_line + 24 - _height;
        bn::fixed aux = (index > _height - 24)*(22*quarter_circle[qc_index]);
        bn::pair<bn::fixed, bn::fixed> left_right(_x - jv::Common::Camera().x() - (_width - aux), _x - jv::Common::Camera().x() + (_width - aux));
...
void jv::Fog::create_room(bn::rect room){
    _rooms.push_back(room);
}

void jv::Fog::set_boundaries(int w, int a){
    _internal_window.restore_boundaries();
    _width = w;
    _height = a;
    bn::fixed cam_y = jv::Common::Camera().y();
