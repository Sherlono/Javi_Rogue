...
void stairs::set_position(bn::point p){
    ...
    Global::Map().insert_map(GameMap(4, 4, (uint8_t*)blocks::data[isOpen ? 33 : 32]), top_left);
}
...
void stairs::set_open(bool open){
    ...
    if(open){
        bn::point top_left((_rect.position().x()>>3)-2, (_rect.position().y()>>3)-2);
        Global::Map().insert_map(GameMap(4, 4, (uint8_t*)blocks::data[33]), top_left);
    }else{
        bn::point top_left((_rect.position().x()>>3)-2, (_rect.position().y()>>3)-2);
        Global::Map().insert_map(GameMap(4, 4, (uint8_t*)blocks::data[32]), top_left);
    }
}
